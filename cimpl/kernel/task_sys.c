 ...
 *
 * Copyright (C) 2021, Eric Londo, <londoed@protonmail.com>, { kernel/task_sys.c }.
...
__fork(struct Task *current, Pid pgrp)
{
    ...
    if (new)
        kprintf(KERN_TRACE, "New taskL %d\n", new->pid);
    else
...
sys_exit(int code)
{
     ...
     * rescheduled. We're deleting the majority of our task info, and a
    ...
    **/
    ...
    /**
     * We're going to delete our address space, including our page table, so
     * we need to switch the kernel's to ensure we don't attempt to keep using
    ...
    **/
    ...
    /**
     * Compiler barrier guarantees taht t->parent will not be read __before__
    ...
    **/
    ...
    /**
     * If we're a kernel process, we will never wait()'d on, so we mark
     * ourselves dead.
    **/
    if (flag_test(&t->flags, TASK_FLAG_KERNEL))
        sched_task_mark_dead();
...
sys_waitpid(Pid childpid, struct UserBuffer wstatus, int options)
{
    ...
    /**
     * We enter a `sleep loop` here. We sleep until we're woken up directly,
    ...
    sleep_intr {
        ...
        using_spinlock(&t->children_list_lock) {
            ...
            list_foreach_entry(&t->task_children, child, task_sibling_list) {
                ...
                if (!(options & WUNTRACED) && !(options & WCONTINUTED))
                    continue;

                kprintf(KERN_TRACE, "Checking %d for continue\n", child->pid);

                if ((child->ret_signal & TASK_SIGNAL_CONT) && (options & WCONTINUTED)) {
                    kprintf(KERN_TRACE, "Found continue child: %d\n", child->pid);
                    ret_status = WCONTINUTED_MAKE();
                    child->ret_signal = 0;
...
sys_dup(int oldfd)
{
    ...
    int newfd, ret = fd_get_checked(oldfd, &filp);

    if (ret)
        return ret;

    newfd = fd_get_empty();
    fd_assign(new_fd, file_dup(filp));
...
sys_dup2(int oldfd, int newfd)
{
    ...
    int ret = fd_get_checked(oldfd, &old_filp);
