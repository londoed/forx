 ...
 *
 * Copyright (C) 2021, Eric Londo <londoed@protonmail.com>, { kernel/sys_user.c }.
...
int
__credentials_belong_to_gid(struct Credentials *creds, Gid gid)
{
    if (creds->egid == gid)
        return 1;
...
int
sys_setresuid(Uid ruid, Uid euid, Uid suid)
{
    struct Credentials *creds = &cpu_get_local()->current->creds;

    using_creds(creds) {
        if (creds->euid != 0) {
            if (ruid != UID_INVALID && creds->uid != ruid && creds->euid != ruid &&
                creds->suid != ruid)
                    return -EPERM;

            if (euid != UID_INVALID && creds->uid != euid && creds->euid != euid &&
                creds->suid != euid)
                    return -EPERM;
        }
...
sys_setregid(Gid rgid, Gid egid)
{
    ...
    using_creds(creds) {
        ...
        if (egid != GID_INVALID) {
            ...
            if (creds->egid != 0 && creds->gid != egid && creds->egid != egid &&
                creds->sgid != egid)
                    return -EPERM;
        ...
        if (creds->gid != GID_INVALID || (creds->egid != GID_INVALID &&
            creds->egid != old_rgid))
...
int
sys_setgroups(size_t size, struct UserBuffer list)
{
    struct Credentials *creds = &cpu_get_local()->current->creds;
...
sys_getgroups(size_t size, struct UserBuffer list)
{
    ...
    for (i = 0; i < NGROUPS && new_sup_groups[i] != GID_INVALID; i++) {
        ...
        int ret = user_copy_to_kernel_indexed(list, new_sup_groups[i], i);
