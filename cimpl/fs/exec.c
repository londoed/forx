 ...
 *
 * Copyright (C) 2021, Eric Londo <londoed@protonmail.com>, { fs/exec.c }.
...
generate_task_name(char *name, size_t len, const char *file, struct UserBuffer argv_buf)
{
    ...
    for (arg = user_buffer_index(argv_buf, sizeof(char *)); !user_buffer_is_null(arg);
...
static int
execve(struct Inode *ino, const char *file, struct UserBuffer argv_buf, struct UserBuffer envp_buf,
    ...
    struct IrqFrame *frame)
{
    ...
    ret = check_permission(exec, X_OK);
